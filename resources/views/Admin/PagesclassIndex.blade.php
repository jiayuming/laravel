...
<blockquote class="layui-elem-quote news_search">
    <div class="layui-inline">
        <a class="layui-btn layui-btn-normal rolesAdd_btn">添加分类</a>
    </div>
    <div class="layui-inline">
...
<script>
    layui.config({
        ...
        //加载页面数据
        var usersData = '';
        $.post("{{ url('admin/pagesclass') }}",{'_token':csrftoken}, function(data){
            usersData = data;
            //执行加载数据的方法
            usersList();
        })
        ...
        $(".rolesAdd_btn").click(function(){
            var index = layui.layer.open({
                title : "添加分类",
                type : 2,
                content : "{{ url('admin/pagesclass/add') }}",
        ...
        $("body").on("click",".users_edit",function(){  //编辑
            ...
            var index = layui.layer.open({
                title : "修改分类信息",
                type : 2,
                content : "{{ url('admin/pagesclass/edit') }}/"+_this.attr("data-id"),
                success : function(layero, index){
                    layui.layer.tips('点击此处返回分类列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
                    });
        ...
        $("body").on("click",".pages_del",function(){  //删除
            ...
            layer.confirm('确定删除此分类？',{icon:3, title:'提示信息'},function(index){
                $.post('{{ url("admin/pagesclass/destroy") }}/'+_this.attr("data-id"),{'_token':csrftoken},function (data) {
                    if(data.status==1){
                        layer.msg(data.msg);
                        layer.close(index);
                ...
                if(currData.length != 0){
                    for(var i=0;i<currData.length;i++) {
                        ...
                        dataHtml += '<tr>'
                            ...
                            + '<td>'
                            + '<a class="layui-btn layui-btn-mini users_edit" data-id="' + currData[i].class_id + '"><i class="fa fa-pencil-square-o"></i> 编辑</a>'
                            + '<a class="layui-btn layui-btn-danger layui-btn-mini pages_del" data-id="' + currData[i].class_id + '"><i class="layui-icon">&#xe640;</i> 删除</a>'
                            + '</td>'
                            +'</tr>';
                    }
                }else{
