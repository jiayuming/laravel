...
<blockquote class="layui-elem-quote news_search">
    <div class="layui-inline">
        <a class="layui-btn layui-btn-normal rolesAdd_btn">添加角色</a>
    </div>
    <div class="layui-inline">
...
<script>
    layui.config({
        ...
        //加载页面数据
        var usersData = '';
        $.post("{{ url('admin/roles') }}",{'_token':csrftoken}, function(data){
            usersData = data;
            //执行加载数据的方法
            usersList();
        })
        ...
        $("body").on("click",".roles_edit",function(){  //编辑
            ...
            var index = layui.layer.open({
                title : "修改角色信息",
                type : 2,
                content : "{{ url('admin/roles/edit') }}/"+_this.attr("data-id"),
                success : function(layero, index){
                    layui.layer.tips('点击此处返回角色列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
                    });
        ...
        $("body").on("click",".roles_del",function(){  //删除
            ...
            layer.confirm('确定删除此角色？',{icon:3, title:'提示信息'},function(index){
                $.post('{{ url('admin/roles/destroy') }}/'+_this.attr("data-id"),{'_token':csrftoken},function (data) {
                    layer.msg(data);
                    layer.close(index);
                ...
                if(currData.length != 0){
                    for(var i=0;i<currData.length;i++) {
                        dataHtml += '<tr>'
                            ...
                            + '<td>';
                        for(var x= 0;x<currData[i].perms.length;x++){
                            dataHtml +="<span class='layui-btn layui-btn-mini'>"+currData[i].perms[x].display_name+"</span>";
                        }
                        dataHtml +='</td>'
                            + '<td>' + formatTime(currData[i].created_at) + '</td>'
                            + '<td>';
                        if (currData[i].name!='admin'){
                            dataHtml += '<a class="layui-btn layui-btn-mini roles_edit" data-id="' + data[i].id + '"><i class="iconfont icon-edit"></i> 编辑</a>'
                                + '<a class="layui-btn layui-btn-danger layui-btn-mini roles_del" data-id="' + data[i].id + '"><i class="layui-icon">&#xe640;</i> 删除</a>';
                        }
                        dataHtml +=  '</td>'
                            +'</tr>';
                    }
