        ...
        <div class="layui-form layui-inline">
            <select class="search_class">
                ...
                @foreach($classList as $list)
                    <option value="{{ $list->class_id }}">{{ $list->html }}{{ $list->name }}</option>
                    @endforeach
            </select>
        </div>
...
<script>
    layui.config({
        ...
        var dataCount=0;
        var current_page=0;
        $.post("{{ url('admin/pages') }}?page=1",{'_token':'{{ csrf_token() }}'}, function(data){
            usersData = data.data;
        ...
        var staticKeyword='';
        var staticClass='';
        function getContent(mypage,pagekeywords,staticClass) {
            if(staticKeyword){
                pagekeywords=staticKeyword;
            }
            $.post("{{ url('admin/pages') }}?page="+mypage,{'_token':'{{ csrf_token() }}','keywords':pagekeywords,'staticClass':staticClass}, function(data){
                usersData = data.data;
                dataCount = data.total;
                current_page = data.current_page;
                //执行加载数据的方法
        ...
        $(".batchDel").click(function(){
            ...
            if($checkbox.is(":checked")){
                layer.confirm('确定删除选中的信息？',{icon:3, title:'提示信息'},function(index){
                    ...
                    $.post("{{ url('admin/pages/allDelete') }}",{'_token':'{{ csrf_token() }}','ids':checkID}, function(data){
        ...
        $("body").on("click",".pages_edit",function(){  //编辑
            ...
            var index = layui.layer.open({
                title : "修改文章信息",
                type : 2,
                content : "{{ url('admin/pages/edit') }}/"+_this.attr("data-id"),
                success : function(layero, index){
                    layui.layer.tips('点击此处返回文章列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
                    });
        ...
        $("body").on("click",".pages_del",function(){  //删除
            ...
            layer.confirm('确定删除此文章？',{icon:3, title:'提示信息'},function(index){
                $.post('{{ url("admin/pages/destroy") }}/'+_this.attr("data-id"),{'_token':'{{ csrf_token() }}'},function (data) {
                    if(data.status==1){
                        layer.msg(data.msg);
                        layer.close(index);
